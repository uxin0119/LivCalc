...
import { calculateAccuracyModifier, chooseCombatAction, calculateFleeChance, hasSuppressor, COMBAT_CONSTANTS } from '../logic/combat';
import { generateHumanEnemy } from '../logic/enemy';
import { lootTable, areaNames, containerNames, extLogo } from '../constants';
...
export const useGameTick = (params: UseGameTickParams) => {
  const {
    raidStatus,
    missionDuration,
    elapsedTime,
  ...
  useEffect(() => {
    ...
    const gameTick = () => {
        ...
        if (raidStatus === 'evacuating') {
            ...
            if (evacTime <= 1) {
                setLog(prev => [...prev, ...extLogo, `[${formatTime(elapsedTime)}] Extraction successful!`]);
                handleMissionSuccess();
        ...
        for (const partName in newParts) {
            const part = newParts[partName as BodyPart];
